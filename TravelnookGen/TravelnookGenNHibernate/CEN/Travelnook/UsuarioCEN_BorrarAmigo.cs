...
public void BorrarAmigo (string p_yo, string p_amigo)
{
        ...
        foreach (UsuarioEN aux1 in misamigos) { //busco en mi lista de amigos al amigo que quiero borrar
                if (aux1.NomUsu == p_amigo) { //si esta en mi lista de amigos lo borro
                        paraborrar.Add (p_amigo);
                        usuCEN.EliminarAmigo (p_yo, paraborrar);
                        borrado = true;
                }
        }
        if (borrado == false) { //si no esta en mi lista de amigos, yo estoy en su lista
                paraborrar.Add (p_yo);
